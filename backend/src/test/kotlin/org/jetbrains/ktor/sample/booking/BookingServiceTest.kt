...
class BookingServiceTest : DatabaseSpec() {
    ...
    private suspend fun createTestCustomer(
        ...
        email: String = "test.customer@example.com",
    ): Customer {
    ...
    fun `test partial update booking`() = runBlocking {
        ...
        ) // Booking date should remain unchanged
        assertEquals(newAmount, updatedBooking?.amount)
    }
    ...
    fun `test create booking for non-existent customer`() = runBlocking {
        ...
        try {
            ...
            assertTrue(
                false,
                "Expected an exception when creating a booking for a non-existent customer",
            )
        } catch (e: Exception) {
            // Expected exception
    ...
    fun `test update non-existent booking`() = runBlocking {
        ...
        val updatedBooking =
            bookingService.updateBooking(999, bookingDate = bookingDate, amount = amount)
