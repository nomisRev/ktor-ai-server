...
class AiService(module: AiModule, registry: MeterRegistry) {
    ...
    private val gauge =
        Gauge.builder("ai.question.answer.inflight") { inflight.get() }
            .description("Amount of inflight AI questions to answer")
            .register(registry)

    private val questionAnswerTimer =
        Timer.builder("ai.question.answer.time")
            .description("Time taken to answer a question")
            .register(registry)

    fun answer(userId: String, question: String): Flow<String> =
        chat.answer(userId, question).track(::trackAiQuestion)
